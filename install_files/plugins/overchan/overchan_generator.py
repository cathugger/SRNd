...
class OverchanGeneratorTools(OverchanGeneratorInit):
  ...
  def expire_board(self, group_id):
    ...
    for row in self.overchandb.execute('SELECT article_uid FROM articles WHERE parent = "" AND group_id = ? AND article_uid NOT IN ( SELECT article_uid FROM articles WHERE parent = "" ORDER BY sent DESC LIMIT ? )', (group_id, threads)).fetchall():
